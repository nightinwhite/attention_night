...
class Attention_Model(object):
    ...
    def create_model(self,scope='AttentionOcr',
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            chars_logit = tf.transpose(chars_logit, (1, 0, 2))
            net_len_constant = tf.constant(np.full([FLAGS.batch_size], FLAGS.seq_length), dtype=tf.int32)
            ctc_loss = tf.nn.ctc_loss(self._label,chars_logit,net_len_constant)
            ctc_loss = tf.reduce_mean(ctc_loss)
            # ctc_loss = self.sequence_loss_fn(chars_logit,self._label)
            decoded, log_prob = tf.nn.ctc_greedy_decoder(chars_logit, net_len_constant)
            self.predict = decoded[0]
            slim.losses.add_loss(ctc_loss)
